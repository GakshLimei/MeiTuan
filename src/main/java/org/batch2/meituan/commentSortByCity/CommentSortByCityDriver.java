 ...
 * @desc:
 */
public class CommentSortByCityDriver {
    ...
    public static void main(String[] args) throws Exception {
        ...
        Job job = Job.getInstance(conf, CommentSortByCityDriver.class.getSimpleName());

        //设置mr程序运行的主类
        job.setJarByClass(CommentSortByCityDriver.class);

        //设置本次mr程序的mapper类型、reducer类型
        job.setMapperClass(CommentSortByCityMapper.class);
        job.setReducerClass(CommentSortByCityReducer.class);

        //指定mapper阶段输出的key value数据类型
        job.setMapOutputKeyClass(CommentSortByCityBean.class);
        job.setMapOutputValueClass(Text.class);

        //指定reducer阶段输出的key value数据类型，也是mr程序最终的输出数据类型
        job.setOutputKeyClass(Text.class);
        job.setOutputValueClass(CommentSortByCityBean.class);


        //配置本次作业的输入数据路径和输出数据路径
        Path inputPath = new Path("output/meituan/sumbycity");
        Path outputPath = new Path("output/meituan/commentsortbycity");
