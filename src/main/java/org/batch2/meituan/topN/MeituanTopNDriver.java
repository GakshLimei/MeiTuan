...
public class MeituanTopNDriver {

    public static void main(String[] args) throws Exception {
        //配置文件对象
        Configuration conf = new Configuration();
        //创建作业实例
        Job job = Job.getInstance(conf, MeituanTopNDriver.class.getSimpleName());
        //设置作业驱动类
        job.setJarByClass(MeituanTopNDriver.class);
        //设置作业mapper reducer
        job.setMapperClass(MeituanTopNMapper.class);
        job.setReducerClass(MeituanTopNReducer.class);
        ...
        //配置作业的输出数据路径
        FileOutputFormat.setOutputPath(job, new Path("output/meituan/topN"));
        //判断输出路径是否存在 如果存在删除
